...
export default function Sidebar({ open, onClose, onCategoryChange, onPostClick, onProfileClick }) {
  ...
  const sideStyle = {
    ...
    overflowY: 'auto', direction: isRTL ? 'rtl' : 'ltr',
  ...
  const Row = ({ icon, label, onClick, color }) => (
    <div
      onClick={onClick}
      style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 20px', cursor: 'pointer', borderBottom: '1px solid #f0f0f0', background: 'white', transition: 'background 0.15s' }}
  ...
  return (
    <>
      ...
      <div style={sideStyle}>
        ...
        {/* Header */}
        <div style={{ background: 'linear-gradient(135deg,#E8192C,#c0392b)', padding: '20px 16px' }}>
        ...
        <div onClick={() => { onPostClick && onPostClick(); onClose() }}
          style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 20px', cursor: 'pointer', background: 'linear-gradient(135deg,#FFF0F1,#fff5f5)', borderBottom: '2px solid #E8192C' }}>
        ...
        <div onClick={() => setShowCats(!showCats)}
          style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 20px', cursor: 'pointer', borderBottom: '1px solid #f0f0f0', background: showCats ? '#f9fafb' : 'white' }}>
        ...
        {showCats && CATEGORIES.map(cat => (
          <div key={cat.key} onClick={() => handleCat(cat.key)}
            style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 20px 11px 56px', cursor: 'pointer', borderBottom: '1px solid #f9fafb', background: '#fafafa' }}
            onMouseEnter={e => e.currentTarget.style.background = '#FFF0F1'}
        ...
        {showLang && LANGS.map(l => (
          <div key={l.code} onClick={() => { setLang(l.code); setShowLang(false) }}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 20px 11px 56px', cursor: 'pointer', borderBottom: '1px solid #f9fafb', background: lang === l.code ? '#FFF0F1' : '#fafafa' }}
            onMouseEnter={e => e.currentTarget.style.background = '#FFF0F1'}
